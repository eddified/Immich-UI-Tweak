...
function assertStrictDemoMappedPathDisplay(exp: Expect, text: string | null): void {
  const t = text?.trim() ?? '';
  exp(/\/data\/upload/i.test(t), 'mapped path must not contain /data/upload').toBe(false);
  exp(t, 'mapped path must match /var/test/<uuid>/<hex>/<hex>/<uuid>.<ext>').toMatch(
...
async function saveExtensionOptions(
  ...
  await applyDemoExtensionSettings(page, extensionId, protocol, pathMapping);
  await page.close();
}
...
function expectFoldersHrefUsesServerPath(exp: Expect, href: string | null, baseOrigin: string): void {
  exp(href).toBeTruthy();
  const u = new URL(href!, baseOrigin);
  const p = u.searchParams.get('path');
  exp(p).toBeTruthy();
  exp(p!.startsWith('/data')).toBe(true);
}

export function registerImmichDemoTests(
  ...
  test('partner timeline: uploader overlay on Jan 22 first thumbnail', async ({
    ...
    await saveExtensionOptions(page, extensionId, protocol);
  ...
  test('search results: first thumbnail has uploader overlay after refresh', async ({ context, extensionId }) => {
    const page = await context.newPage();
    await applyExtensionConfig(page, extensionId, protocol, {
      enabledOrigin: demoOrigin(),
      pathMappings: [{ localPath: '/var/test', immichPath: '/data/upload' }],
      showPartnerIcons: true,
      showOwnProfileIcon: true,
  ...
  test('video search: uploader overlay does not cover Immich thumbnail icons', async ({
    ...
    const firstThumb = appPage
      .locator('[data-asset]')
      .filter({ has: appPage.locator('img[alt*="Video taken"]') })
      .first();
    await expect(firstThumb).toBeVisible({ timeout: 30_000 });
  ...
  test('photo view: mapped file path visible in info panel', async ({ context, extensionId }) => {
    ...
    assertStrictDemoMappedPathDisplay(expect, await mappedLink.textContent());
  });
  ...
  test('photo view: mapped path updates on next photo (local roots without leading slash)', async ({
    ...
    await saveExtensionOptionsForSite(page, extensionId, protocol, [
  ...
  test('folders page: breadcrumb or tree link href keeps server path in query', async ({
    ...
    expectFoldersHrefUsesServerPath(expect, await link.getAttribute('href'), DEMO);
  });
  ...
  test('info panel: Google Maps link hidden when extension option off', async ({ context, extensionId }) => {
    ...
    await applyExtensionConfig(optPage, extensionId, protocol, {
      enabledOrigin: demoOrigin(),
      pathMappings: [{ localPath: '/var/test', immichPath: '/data/upload' }],
      showPartnerIcons: true,
      replaceFoldersPageNames: true,
      googleMapsLinkInInfoPanel: false,
    });
    await optPage.close();
  ...
  test('info panel: Google Maps embed iframe when replace-OSM option on', async ({ context, extensionId }) => {
    ...
    await applyExtensionConfig(optPage, extensionId, protocol, {
      ...
      showPartnerIcons: true,
      replaceFoldersPageNames: true,
      googleMapsEmbedInsteadOfOsmInInfoPanel: true,
    });
    await optPage.close();
  ...
  test('info panel: edit faces panel covers extension toolbar and Google Maps embed', async ({
    ...
    await applyExtensionConfig(optPage, extensionId, protocol, {
      ...
      replaceFoldersPageNames: true,
      googleMapsEmbedInsteadOfOsmInInfoPanel: true,
      infoPanelDetailRowFile: 'collapse',
  ...
  test('info panel: file path stays hidden after user toggles it off', async ({ context, extensionId }) => {
    ...
    await expect(pathLink).toBeVisible({ timeout: 15_000 });
    assertStrictDemoMappedPathDisplay(expect, await pathLink.textContent());
