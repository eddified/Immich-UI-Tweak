...
import { hasHostPermissionsForEnabledUrls } from './shared/host-permissions';
...
async function syncRegisteredContentScripts(): Promise<void> {
  ...
  const permitted = await hasHostPermissionsForEnabledUrls(enabledUrls);
  if (!permitted) {
    console.warn(
      'immich-ui-tweak: host permission not granted — open Options and click “Allow on Immich URLs”.',
    );
    return;
  }
  ...
  try {
    ...
    console.error('immich-ui-tweak: registerContentScripts (main+isolated) failed:', err);
    /* Older browsers without `world` on registerContentScripts — isolated + message fallback */
    try {
      await chrome.scripting.registerContentScripts([
        {
          id: CONTENT_SCRIPT_ID,
      ...
      console.error('immich-ui-tweak: registerContentScripts (isolated only) failed:', fallbackErr);
    }
  }
}
